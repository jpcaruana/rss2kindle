...
def print_error(exc_type, feed, feednum, http_headers, http_result, http_status):
    if http_status not in [200, 302]:
        print ("W: error %d [%d] %s" % (http_status, feednum, feed.url))

    elif contains(http_headers.get('content-type', 'rss'), 'html'):
        print ("W: looks like HTML [%d] %s" % (feednum, feed.url))

    elif http_headers.get('content-length', '1') == '0':
        print ("W: empty page [%d] %s" % (feednum, feed.url))

    elif hasattr(socket, 'timeout') and exc_type == socket.timeout:
        print ("W: timed out on [%d] %s" % (feednum, feed.url))

    elif exc_type == IOError:
        print ('W: "%s" [%d] %s' % (http_result.bozo_exception, feednum, feed.url))

    elif hasattr(feedparser, 'zlib') and exc_type == feedparser.zlib.error:
        print ("W: broken compression [%d] %s" % (feednum, feed.url))

    elif exc_type in socket_errors:
        exc_reason = http_result.bozo_exception.args[1]
        print ("W: %s [%d] %s" % (exc_reason, feednum, feed.url))
        ...
        print ('E: error in [%d] "%s" feed (%s)' % (
                    feednum, feed.url, http_result.get("bozo_exception", "can't process")))

    else:
        print ("=== rss2kindle encountered a problem with this feed ===")
        print ("E:", http_result.get("bozo_exception", "can't process"), feed.url)
        print (http_result)
        print ("rss2kindle", __version__)
    ...
    try:
        for feednum, feed in enumerate(ifeeds(feeds, num)):
            try:
                ...
                if VERBOSE:
                    print ('I: Processing [%d] "%s"' % (feednum, feed.url))
                http_result = {}
                try:
                    http_result = timelimit(FEED_TIMEOUT, parse)(feed.url, feed.etag, feed.modified)
                except TimeoutError:
                    print ('W: feed [%d] "%s" timed out' % (feednum, feed.url))
                    continue
                ...
                if 'status' in http_result:
                    if http_result.status == 301:
                        ...
                        print ("W: feed gone; deleting", feed.url)
                        feeds.remove(feed)
                        continue

                http_status = http_result.get('status', 200)
                if VERBOSE > 1:
                    print ("I: http status", http_status)
                ...
                print ("=== rss2kindle encountered a problem with this feed ===")
                print ("E: could not parse", feed.url)
...
def delete(n):
    ...
    if (n == 0) and (feeds and isstr(feeds[0])):
        print ("W: ID has to be equal to or higher than 1")
    elif n >= len(feeds):
        print ("W: no such feed")
    else:
        print ("W: deleting feed %s" % feeds[n].url)
        feeds = feeds[:n] + feeds[n + 1:]
        if n != len(feeds):
            print ("W: feed IDs have changed, list before deleting again")
    unlock(feeds, feedfileObject)
...
def toggleactive(n, active):
    ...
    if (n == 0) and (feeds and isstr(feeds[0])):
        ...
        print ("%s feed %s" % (action, feeds[n].url))
